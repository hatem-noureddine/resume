...
async function buildAdminTools() {
    ...
    if (fs.existsSync(ADMIN_DIR)) {
        console.log('🧹 Cleaning existing public/admin directory...');
    ...
    if (fs.existsSync(nextAnalyzeDir)) {
        ...
        console.warn('   Run "ANALYZE=true npm run build --webpack" to generate them.');
    }
    ...
    try {
        ...
        // We use --quiet to reduce logs, and -o to specify output
        execSync(`npx storybook build --quiet`, { stdio: 'inherit' });
