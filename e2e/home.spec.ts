...
test.describe('Homepage Responsive & Functionality', () => {
    ...
    test('should display desktop navigation correctly', async ({ page, isMobile }) => {
        ...
        await expect(page.getByRole('heading', { name: /Professional|Skills/i })).toBeVisible({ timeout: 10000 });

        // Check navigation links
        const nav = page.locator('nav.md\\:flex');
        await expect(nav).toBeVisible();
        await expect(nav.getByText('Services')).toBeVisible();
        await expect(nav.getByText('Projects').or(page.getByText('Portfolio'))).toBeVisible();
    });
    ...
    test('should switch skills tabs on mobile', async ({ page, isMobile }) => {
        if (!isMobile) test.skip();
        ...
        // Check Tab Buttons
        const professionalTab = page.getByText('Professional', { exact: true });
        const technicalTab = page.getByText('Technical', { exact: true });

        await expect(professionalTab).toBeVisible();
        await expect(technicalTab).toBeVisible();

        // Default: Professional visible
        // On mobile, headers might be hidden or different structure.
        // Just check content
        await expect(page.getByText('Team Management')).toBeVisible();

        // Click Technical Tab
        await technicalTab.first().click();

        // Verify Technical content is visible (Accordion)
    ...
    test('should not have horizontal scroll', async ({ page, isMobile }) => {
        ...
        expect(scrollWidth).toBeLessThanOrEqual(clientWidth + 1); // +1 buffer for sub-pixel rendering
    });
});
